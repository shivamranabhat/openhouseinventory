<div id="zero-config_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4">
        ...
        <table id="zero-config" class="table table-striped dt-table-hover dataTable" style="width:100%" role="grid"
            ...
            <tbody wire:poll.keep-alive>
                ...
                <tr role="row">
                    ...
                    <td>{{$graph->tag_name}}</td>
                    <td>{{$graph->page->name}}</td>
                    <td> <span class="inv-date"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            </svg> {{\Carbon\Carbon::parse($graph->created_at)->format('M d Y')}} </span></td>
                    <td class="d-flex">
                        @if($confirmingDeletion === $graph->id)
                        <!-- Show Yes and No buttons -->
                        <div class="d-flex gap-2">
                            <button wire:click="delete({{$graph->id}})" class="btn badge badge-danger">Yes</button>
                        ...
                        <a class="badge badge-light-primary text-start me-2 action-edit"
                            href="{{route('graph.edit', $graph->slug)}}">
                        ...
                        <a class="badge badge-light-danger text-start action-delete"
                            wire:click="confirmDelete({{$graph->id}})" role="button">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
    ...
    {{$graphs->links('vendor.pagination.pagination')}}
    <x-success />
    <x-error />
</div>